...
0_4_StatsRerank_B.py — Loto 7/39 NEXT predikcija

Mod B = soft statistički rerank.
...
Zatim se iz top kandidata generišu kombinacije i rerankuju po formuli:
  ...
  total_score = model_score + STATS_WEIGHT * stats_score

gde stats_score nagrađuje kombinacije čije su:
  suma, broj neparnih, broj niskih (<=19), raspon
bliže istorijskom proseku.
...
OUT_TXT = "/0_4_StatsRerank_B_predikcija.txt"
PLOT_PATH = "/0_4_SkewnessKurtosis_B.png"
...
T0 = time.time()
print()
print("🔁 0_4_StatsRerank_B — start ", stamp())
print()
...
print("📐 Mod B statistički prior (mean/std):")
...
# ============================================================
# 5) Rerank B: soft statistički score
# ============================================================
...
def rerank_b(scores_1d: np.ndarray) -> tuple[np.ndarray, float, float]:
    ...
    for combo_tuple in itertools.combinations(top_pool, K):
        ...
        total_score = model_score + STATS_WEIGHT * stat_score
        if total_score > best_total:
            best_total = total_score
            best_stats = stat_score
            best_combo = combo

    return best_combo, best_total, best_stats
...
def avg_hits(scores_2d, Y, use_rerank: bool) -> float:
    ...
    for i in range(scores_2d.shape[0]):
        ...
        pred = rerank_b(scores_2d[i])[0] if use_rerank else topk_from_scores(scores_2d[i])
...
print("📊 Tabela rezultata — mod B (soft statistički rerank)")
print(f"{'model':<5} {'raw_pick':<31} {'rerank_B':<31} {'raw_h':>6} {'B_h':>6} {'AUC':>7} {'LRAP':>7} {'stat':>8}")
...
for name, model in models.items():
    ...
    rerank_pick, total_score, stat_score = rerank_b(scores_next)
    ...
    print(
        ...
        f"{raw_h:>6.3f} {rerank_h:>6.3f} {auc:>7.3f} {lrap:>7.3f} {stat_score:>8.3f}"
    )
    txt_lines.append(
        f"{name}: raw={raw_pick.tolist()} ({describe(raw_pick)}); "
        f"rerank_B={rerank_pick.tolist()} ({describe(rerank_pick)}); "
        f"raw_hits={raw_h:.3f}; rerank_hits={rerank_h:.3f}; "
        f"AUC={auc:.3f}; LRAP={lrap:.3f}; stat_score={stat_score:.3f}; total_score={total_score:.3f}"
    )
...
with open(OUT_TXT, "a", encoding="utf-8") as f:
    f.write(f"\n--- {stamp()} (seed={SEED}, N={N}, mode=B, top_pool={TOP_POOL}, weight={STATS_WEIGHT}) ---\n")
...
print("🔁 0_4_StatsRerank_B — stop ", stamp())
...
"""
🔁 0_4_StatsRerank_B — start  28.05.2026_11.14.15

✅ CSV učitan: /data/loto7_4622_k42.csv
   broj izvlačenja: 4622, brojeva po kolu: 7

📐 Mod B statistički prior (mean/std):
...
📊 Tabela rezultata — mod B (soft statistički rerank)
model raw_pick                        rerank_B      raw_h    B_h     AUC    LRAP     stat
DTR   [8, 13, 16, 23, 31, 34, 37]     [5, 8, 13, 23, 25, 32, 34]     1.480  1.300   0.506   0.254   -0.247
RFR   [7, 8, 23, 26, 27, 32, 35]      [3, 7, 8, 26, 29, 32, 33]      1.170  1.230   0.492   0.245   -0.223
XGB   [2, 7, 23, 28, 30, 32, 37]      [2, 7, 15, 23, 28, 30, 32]     1.140  1.210   0.498   0.239   -0.360

(slučajan baseline ≈ 1.256 hits/7)

📝 Snimljeno u: /0_4_StatsRerank_B_predikcija.txt
🖼️  Plot snimljen u: /0_4_SkewnessKurtosis_B.png

🔁 0_4_StatsRerank_B — stop  28.05.2026_11.15.49
⏱️  Ukupno vreme: 0:01:33  (94.0 s)
"""
